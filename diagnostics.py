...
def calculate_montage_pm_residuals(mont_pm, z_start = None, z_end = None, normalizer = 1, method = 'std', cutoff = 3, greater_than = True, verbose = True, **kwargs):
    ...
    group_ids = ren.pointmatch.get_match_groupIds_from_only(mont_pm['matchSource'], render=r)
    ...
    for j in range(z_len): # 
        ...
        matches = ren.pointmatch.get_matches_within_group(mont_pm['matchSource'],group_id, render=r)
...
def calculate_area_perimeter_ratios(raw, montaged, z_start = None, z_end = None,
    ...
    Within each of the above dictionaries:
        ...
        'outlier counts' -- number of outliers specified by chosen method, array
    ...
    area_dict = {'target values':Zs_area_target, 'ratios':Zs_area_ratio, 'means':Zs_area_mean, 'variances':Zs_area_variance, 'medians':Zs_area_median, 'outlier counts':Zs_area_outlier_count, 'outlier ids': Zs_area_outlier_ids, 'outlier percent':Zs_area_outlier_percent, 'outlier indecies':Zs_area_outlier_idx}
    perimeter_dict = {'target values':Zs_perimeter_target, 'ratios':Zs_perimeter_ratio, 'means':Zs_perimeter_mean, 'variances':Zs_perimeter_variance, 'medians':Zs_perimeter_median, 'outlier counts':Zs_perimeter_outlier_count, 'outlier ids': Zs_perimeter_outlier_ids, 'outlier percent':Zs_perimeter_outlier_percent, 'outlier indecies':Zs_perimeter_outlier_idx}
